...
class RequestHandler(object):
    ...
    async def __call__(self, request):
        print('ruaruarua')
        print(type(request))
        print('request.match_info.items()============',request.match_info.items())
        kw = None
        if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
            ...
            if request.method == 'GET':
                #query_strings是get方法的ur后面附带的键值对
                qs = request.query_string
                #把query_string转换成dict
                if qs:
                    kw = dict()
        ...
        if kw == None:
            ...
            for k, v in request.match_info.items():
                if k in kw:
                    logging.warning('Duplicate aeg in named arg and kw args: %s' % k)
                kw[k] = v
...
#   print(type(inspect.signature(x)))
